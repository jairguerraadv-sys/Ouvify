...
shell-db: ## Abrir psql shell
	docker-compose exec postgres psql -U ouvy -d ouvy_dev
...
backup-db: ## Backup do banco de dados
	docker-compose exec postgres pg_dump -U ouvy ouvy_dev > backup_$$(date +%Y%m%d_%H%M%S).sql

restore-db: ## Restore backup (make restore-db FILE=backup.sql)
	cat $(FILE) | docker-compose exec -T postgres psql -U ouvy ouvy_dev
