...
function MyPage() {
  ...
  return <div>{theme?.logo ? <img src={theme.logo} /> : <DefaultLogo />}</div>;
}
```
    ...
    <footer className="bg-muted py-8 mt-auto">
      <div className="container mx-auto text-center">
        ...
        <p className="text-sm text-muted-foreground">
          © {new Date().getFullYear()} {theme?.nome || "Ouvify"}. Todos os
          direitos reservados.
        </p>
      </div>
    </footer>
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      {showName && theme?.nome && theme.nome !== "Ouvify" && (
        <span className="text-lg font-semibold text-primary">{theme.nome}</span>
      )}
    </div>
  );
}
...
export function BrandLogoSkeleton({ size = "md" }: { size?: string }) {
  ...
  return <Skeleton className={`w-32 ${heights[size]}`} />;
}
...
function Header() {
  ...
  return (
    <header>
      {!theme ? <BrandLogoSkeleton size="md" /> : <BrandLogo size="md" />}
    </header>
  );
}
```
