...
contrastTests.forEach(test => {
  ...
  console.log(`${test.bg} × ${test.fg}: ${ratio.toFixed(2)}:1 ${ratio >= test.min ? '✅' : '❌'}`)
})
```
...
@layer base {
  * {
  ...
  h1 { @apply text-5xl tracking-tight; }
  h2 { @apply text-4xl tracking-tight; }
  h3 { @apply text-3xl; }
  h4 { @apply text-2xl; }
  h5 { @apply text-xl; }
  h6 { @apply text-lg; }
...
interface StyleIssue {
  file: string
  line: number
  type: 'inline-style' | 'duplicate-class' | 'hardcoded-color' | 'deprecated-class'
  description: string
}
...
<div className="space-y-6">
  ...
  {/* Lista */}
  <div className="space-y-4">
    {items.map(item => <FeedbackCard key={item.id} {...item} />)}
  </div>
</div>
```
