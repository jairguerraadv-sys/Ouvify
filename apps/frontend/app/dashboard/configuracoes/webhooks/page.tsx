...
function WebhooksContent() {
  ...
  const loadWebhooks = async () => {
    try {
      const response = await api.get('/webhooks/endpoints/') as { data: { results?: WebhookEndpoint[] } | WebhookEndpoint[] };
  ...
  const loadStats = async () => {
    try {
      const response = await api.get('/webhooks/endpoints/stats/') as { data: WebhookStats };
  ...
  const loadAvailableEvents = async () => {
    try {
      const response = await api.get('/webhooks/endpoints/available_events/') as { data: AvailableEvent[] };
  ...
  const loadDeliveries = async (webhookId: string) => {
    try {
      const response = await api.get(`/webhooks/endpoints/${webhookId}/deliveries/`) as { data: WebhookDelivery[] };
      setDeliveries(prev => ({
        ...prev,
  ...
  const toggleWebhook = async (id: string, currentState: boolean) => {
    try {
      await api.patch(`/webhooks/endpoints/${id}/`, {
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{stats.total_events_today}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">Crie seu primeiro webhook para começar a receber notificações</p>
              </div>
            ) : (
          ...
          <CardContent className="py-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <h4 className="font-medium text-blue-900">Documentação de Webhooks</h4>
                <p className="text-sm text-blue-700">
                  Aprenda como integrar webhooks com sua aplicação
                </p>
              </div>
              <Button variant="outline" className="border-blue-300 text-blue-700 hover:bg-blue-100">
                <a href="/docs/webhooks" target="_blank">Ver Documentação</a>
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="p-4 bg-white">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" onClick={onTest} title="Enviar teste">
      ...
      {expanded && (
        <div className="border-t bg-slate-50 p-4 space-y-4">
          ...
          <div>
            ...
            {deliveries.length === 0 ? (
              <p className="text-sm text-muted-foreground mt-2">Nenhuma entrega registrada</p>
            ) : (
