...
import { useSubscriptionStatus, useBillingActions, SubscriptionStatus } from '@/hooks/use-billing';
...
export function SubscriptionManager({ className }: SubscriptionManagerProps) {
  ...
  if (!status?.has_subscription) {
    return <NoSubscriptionCard onUpgrade={() => window.location.href = '/precos'} />;
  }
  ...
  return (
    <Card className={className}>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
            ...
            <div className="text-right">
              <p className="font-bold text-2xl">
                {status.plan.is_free ? 'Grátis' : `R$ ${status.plan.price_cents / 100}`}
              </p>
              {!status.plan.is_free && <p className="text-sm text-muted-foreground">/mês</p>}
            </div>
          </div>
        )}
        ...
        {status.is_trialing && (
          <TrialAlert 
            daysRemaining={calculateTrialDaysRemaining(status)} 
          />
        )}
        ...
        {/* Usage Preview */}
        {status.plan?.limits && (
          <UsagePreview limits={status.plan.limits} />
        )}
      </CardContent>
    </Card>
  );
      ...
      <AlertDescription>
        {daysRemaining > 0 ? (
          <>
            ...
            {daysRemaining <= 3 && 'Adicione um método de pagamento para continuar usando.'}
          </>
        ) : (
...
function UsagePreview({ limits }: { limits: Record<string, number> }) {
  ...
  return (
    <div className="space-y-4">
      <h4 className="font-semibold text-sm text-muted-foreground uppercase">Uso</h4>
