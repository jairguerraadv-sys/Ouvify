...
import React, { ReactNode, useState, type CSSProperties } from "react";
...
export function Alert({
  ...
  return (
    <div className={cn("flex items-start gap-4 p-4 rounded-lg border", variantClasses[variant], className)}>
...
export function Progress({
  ...
  const widthStyle = { width: `${percentage}%` } as CSSProperties;
  ...
  return (
    <div className={cn("space-y-2", className)}>
      <div className={cn("w-full bg-neutral-200 rounded-full overflow-hidden", sizeClasses[size])}>
        <div className={cn("h-full rounded-full transition-all duration-500 ease-out", variantClasses[variant])} style={widthStyle} />
      </div>
      {showLabel && <div className="text-sm font-medium text-neutral-600">{Math.round(percentage)}%</div>}
    </div>
  );
}
    ...
    <div
      className={cn(
        ...
        highlighted
          ...
          : "border-neutral-200 bg-background shadow-subtle hover:shadow-lg",
        className
      )}
    >
      {badge && highlighted && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2">
          <span className="bg-gradient-to-r from-primary to-accent text-text-primary px-4 py-1 rounded-full text-sm font-bold">
            {badge}
          </span>
        </div>
      )}
        ...
        {cta && (
          <button
            ...
            className={cn(
              ...
              highlighted
                ? "bg-gradient-to-r from-primary to-accent text-text-primary hover:shadow-lg"
              ...
              >
                <div
                  className={cn(
                    ...
                    isIncluded ? "bg-success/10 text-success" : "bg-neutral-200"
                  )}
                >
                  {isIncluded && "✓"}
                </div>
