...
from apps.core.utils import set_current_tenant, get_current_tenant
...
def test_protocolo_generation():
    ...
    print(f"✓ Formato correto: {'✅ SIM' if all(p.startswith('OUVY-') and len(p) == 14 for p in protocolos) else '❌ NÃO'}")
...
def test_database_indices():
    ...
    print(f"\n📋 Campo 'protocolo':")
    print(f"   ✓ db_index: {'✅ SIM' if getattr(protocolo_field, 'db_index', False) else '❌ NÃO'}")
    print(f"   ✓ unique: {'✅ SIM' if getattr(protocolo_field, 'unique', False) else '❌ NÃO'}")
    print(f"   ✓ editable: {'❌ SIM (problema)' if getattr(protocolo_field, 'editable', True) else '✅ NÃO'}")
