...
# - POST        /api/feedbacks/{id}/adicionar-interacao/ (action híbrida: AllowAny, valida tenant + protocolo p/ anônimo)
...
router.register(
    r"response-templates", ResponseTemplateViewSet, basename="response-template"
)
...
urlpatterns = [
    path("", home_view, name="home"),  # Rota raiz para teste de multi-tenancy
    ...
    path(
        "api/check-subdominio/", CheckSubdominioView.as_view(), name="check-subdominio"
    ),
    ...
    path(
        "api/logout/all/", LogoutAllDevicesView.as_view(), name="api-logout-all-devices"
    ),
    # Perfil do Usuário
