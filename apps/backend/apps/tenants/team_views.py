...
class InvitationAcceptRateThrottle(AnonRateThrottle):
    """Rate limit para aceite de convite (endpoint público via token)."""

    scope = "invitation_accept"


class TeamMemberViewSet(
    PermissionRequiredMixin, TenantFilterMixin, viewsets.ModelViewSet
):
    """
    ...
    def get_queryset(self):
        ...
        return (
            TeamMember.objects.filter(client=self.request.tenant)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return Response(
            {"detail": "Membro removido com sucesso"}, status=status.HTTP_204_NO_CONTENT
        )
        ...
        stats = {
            ...
            "members_by_role": {
                "owner": members.filter(
                    role=TeamMember.OWNER, status=TeamMember.ACTIVE
                ).count(),
                "admin": members.filter(
                    role=TeamMember.ADMIN, status=TeamMember.ACTIVE
                ).count(),
                "moderator": members.filter(
                    role=TeamMember.MODERATOR, status=TeamMember.ACTIVE
                ).count(),
                "viewer": members.filter(
                    role=TeamMember.VIEWER, status=TeamMember.ACTIVE
                ).count(),
            },
...
class TeamInvitationViewSet(
    ...
    def get_queryset(self):
        ...
        return (
            TeamInvitation.objects.filter(client=self.request.tenant)
    ...
    def perform_create(self, serializer):
        ...
        invitation = serializer.save(
            client=self.request.tenant, invited_by=self.request.user
        )
...
Ouvify - Gestão de Feedbacks White Label
        ...
        try:
            send_mail(
                ...
                from_email=getattr(
                    settings, "DEFAULT_FROM_EMAIL", "noreply@ouvify.com"
                ),
