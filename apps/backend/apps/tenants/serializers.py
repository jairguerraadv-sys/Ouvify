...
class RegisterTenantSerializer(serializers.Serializer):
    ...
    senha = serializers.CharField(
        write_only=True, min_length=8, help_text="Senha (mínimo 8 caracteres)"
    )

    # Dados da Empresa
    nome_empresa = serializers.CharField(max_length=100, help_text="Nome da empresa")
    subdominio_desejado = serializers.SlugField(
        max_length=63, help_text="Subdomínio desejado (ex: minhaempresa)"
    )
...
class ClientBrandingSerializer(serializers.ModelSerializer):
    ...
    def validate_cor_secundaria(self, value):
        ...
        if value and not re.match(r"^#[0-9A-Fa-f]{6}$", value):
            raise serializers.ValidationError(
                "Cor deve estar no formato hexadecimal (ex: #10B981)"
            )
        return value
    ...
    def validate_cor_texto(self, value):
        ...
        if value and not re.match(r"^#[0-9A-Fa-f]{6}$", value):
            raise serializers.ValidationError(
                "Cor deve estar no formato hexadecimal (ex: #1F2937)"
            )
        return value
    ...
    def validate_logo(self, value):
        ...
        if value and not value.startswith(("http://", "https://")):
            raise serializers.ValidationError(
                "URL da logo deve começar com http:// ou https://"
            )
        return value
    ...
    def validate_favicon(self, value):
        ...
        if value and not value.startswith(("http://", "https://")):
            raise serializers.ValidationError(
                "URL do favicon deve começar com http:// ou https://"
            )
        return value
    ...
    def validate_email(self, value):
        ...
        if client:
            if TeamMember.objects.filter(
                user__email=value, client=client, status=TeamMember.ACTIVE
            ).exists():
                raise serializers.ValidationError("Este usuário já é membro da equipe")
            if TeamInvitation.objects.filter(
                email=value, client=client, status=TeamInvitation.PENDING
            ).exists():
                raise serializers.ValidationError(
...
class AcceptInvitationSerializer(serializers.Serializer):
    ...
    def save(self):
        ...
        if created:
            ...
            # Segurança: se o usuário já existe, NÃO permitir login automático
            # apenas com token do convite (prevenir takeover por link vazado).
            if not user.check_password(password):
                raise serializers.ValidationError(
                    {
                        "password": "Senha incorreta para este usuário. Faça login e aceite o convite novamente."
                    }
                )
