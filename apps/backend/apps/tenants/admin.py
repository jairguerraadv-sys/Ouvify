...
class ClientAdmin(admin.ModelAdmin):
    list_display = ('nome', 'subdominio', 'plano', 'subscription_status', 'ativo', 'data_criacao')
    search_fields = ('nome', 'subdominio', 'owner__email')
    list_filter = ('ativo', 'plano', 'subscription_status', 'data_criacao')
    ordering = ('nome',)
    readonly_fields = ('data_criacao', 'data_atualizacao', 'stripe_customer_id', 'stripe_subscription_id')
    ...
    fieldsets = (
        ('Informações Básicas', {
            'fields': ('nome', 'subdominio', 'owner', 'ativo')
        }),
        ('White Label', {
