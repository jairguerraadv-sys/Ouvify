...
class CheckSubdominioRateThrottle(AnonRateThrottle):
    """Rate limit para endpoint público de verificação de subdomínio."""

    scope = "tenant_subdomain_check"
...
class TenantInfoView(APIView):
    ...
    def patch(self, request):
        ...
        # Restringir atualização de branding a OWNER/ADMIN (ou superuser)
        if not getattr(request.user, "is_superuser", False):
            ...
            if not is_owner:
                try:
                    ...
                    return Response(
                        {
                            "detail": "Não foi possível validar permissão para modificar este tenant.",
                            "error": "permission_denied",
                        },
                        status=403,
                    )
        ...
        if serializer.is_valid():
            ...
            return Response(
                ClientPublicSerializer(tenant).data, status=status.HTTP_200_OK
            )
...
class UploadBrandingView(APIView):
    ...
    def post(self, request):
        ...
        # Restringir upload de branding a OWNER/ADMIN (ou superuser)
        if not getattr(request.user, "is_superuser", False):
            is_owner = getattr(tenant, "owner_id", None) == getattr(
                request.user, "id", None
            )
        ...
        result = {"logo_url": None, "favicon_url": None, "errors": []}
...
class RegisterTenantView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"detail": "Erro ao criar conta. Tente novamente.", "error": str(e)},
...
class CheckSubdominioView(APIView):
    ...
    permission_classes = [AllowAny]
    throttle_classes = [CheckSubdominioRateThrottle]
    ...
    def get(self, request):
        ...
        if not subdominio:
            return Response(
                {"detail": "Parâmetro 'subdominio' é obrigatório", "available": False},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class TenantAdminViewSet(viewsets.ModelViewSet):
    ...
    http_method_names = ["get", "head", "options", "patch", "put", "post", "delete"]

    def destroy(self, request, *args, **kwargs):
        """Desativa (soft delete) um tenant. Requer superusuário."""
        if not getattr(request.user, "is_superuser", False):
            return Response(
                {"detail": "Apenas superusuários podem desativar tenants."},
        ...
        logger.info(
            f"🔒 Tenant desativado pelo superadmin | "
    ...
    def metrics(self, request):
        ...
        for plano, preco in PLAN_PRICES_BRL.items():
            count = tenants.filter(
                plano=plano, ativo=True, subscription_status="active"
            ).count()
            mrr += count * preco
        ...
        churn_count = tenants.filter(
            subscription_status="canceled", data_atualizacao__gte=inicio_mes
        ).count()

        total_inicio_mes = tenants.filter(data_criacao__lt=inicio_mes).count()
        churn_rate = (
            (churn_count / total_inicio_mes * 100) if total_inicio_mes > 0 else 0
        )
    ...
    def activity_logs(self, request, pk=None):
        ...
        # Feedbacks criados
        feedbacks = Feedback.objects.filter(client=tenant).order_by("-data_criacao")[
            :20
        ]
        for fb in feedbacks:
            activities.append(
        ...
        # Interações recentes
        interacoes = (
            FeedbackInteracao.objects.filter(feedback__client=tenant)
            .select_related("feedback", "autor")
            .order_by("-data")[:20]
        )
...
class CreateCheckoutSessionView(APIView):
    ...
    def post(self, request):
        ...
        if not plano:
            return Response(
                {"detail": "Campo 'plano' é obrigatório", "error": "missing_plan"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"detail": "Usuário não tem um tenant associado", "error": "no_tenant"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class UserMeView(APIView):
    ...
    def get(self, request):
        ...
        if getattr(user, "is_superuser", False):
            cargo = "Super Admin"
        elif tenant is not None and getattr(tenant, "owner_id", None) == getattr(
            user, "id", None
        ):
            cargo = "Proprietário"
...
class SubscriptionView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response(
                {"detail": "Tenant não encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        return Response(
            {
                ...
                "current_period_start": (
                    tenant.data_criacao.isoformat()
                    if hasattr(tenant, "data_criacao")
                    else now.isoformat()
                ),
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.info(
                f"✅ Assinatura cancelada | Tenant: {tenant.subdominio} | Sub ID: {tenant.stripe_subscription_id}"
            )
