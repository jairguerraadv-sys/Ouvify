...
class JWTAuthTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.tenant = Client.objects.create(
            nome="Test Tenant", subdominio="testjwt", plano="pro", ativo=True
        )
    ...
    def test_authenticated_request_with_jwt(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {access_token}", HTTP_HOST="testjwt.localhost"
        )
        response = self.client.get("/api/users/me/")

        self.assertEqual(response.status_code, 200)

    def test_tenant_mismatch_blocked_with_jwt(self):
        """Usuário autenticado via JWT não pode trocar o tenant pelo host/header."""
        from apps.tenants.models import Client
        ...
        self.client.credentials(
            ...
            HTTP_HOST=f"{other_tenant.subdominio}.localhost",
        )
        response = self.client.get("/api/users/me/")

        self.assertEqual(response.status_code, 403)
        self.assertEqual(response.json().get("error"), "tenant_mismatch")
    ...
    def test_expired_token_rejected(self):
        ...
        access.set_exp(
            from_time=timezone.now() - timedelta(seconds=2),
    ...
    def test_token_auth_header_rejected(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION="Token not-a-real-token", HTTP_HOST="testjwt.localhost"
        )
        response = self.client.get("/api/users/me/")
