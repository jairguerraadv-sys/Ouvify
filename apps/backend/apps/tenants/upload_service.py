...
class UploadService:
    ...
    def validate_mime_type(cls, file) -> Tuple[bool, Optional[str]]:
        ...
        if not MAGIC_AVAILABLE:
            ...
            logger.warning(
                "python-magic não disponível. Validação de MIME type ignorada."
            )
            return True, None
    ...
    @classmethod
    def validate_image(
        cls, file, max_size_mb: int, is_favicon: bool = False
        ...
        if file_ext not in cls.ALLOWED_FORMATS:
            return (
                False,
                f"Formato não suportado. Use: {', '.join(cls.ALLOWED_FORMATS)}",
            )
        ...
        try:
            ...
            if is_favicon:
                ...
                if width > cls.LOGO_MAX_WIDTH or height > cls.LOGO_MAX_HEIGHT:
                    return (
                        False,
                        f"Dimensões muito grandes. Máximo: {cls.LOGO_MAX_WIDTH}x{cls.LOGO_MAX_HEIGHT}px",
                    )
    ...
    def upload_logo(
        ...
        """
        # Validar imagem
        is_valid, error = cls.validate_image(
            file, cls.MAX_LOGO_SIZE_MB, is_favicon=False
        )
        if not is_valid:
        ...
        try:
            ...
            logger.info(
                f"Logo uploaded successfully for tenant {tenant_subdomain}: {url}"
            )
    ...
    def upload_favicon(
        ...
        # Validar imagem
        is_valid, error = cls.validate_image(
            file, cls.MAX_FAVICON_SIZE_MB, is_favicon=True
        )
        if not is_valid:
            return False, None, error
        ...
        try:
            ...
            result = cloudinary.uploader.upload(
                ...
                transformation=[
                    {
                        "width": cls.FAVICON_SIZE,
                        "height": cls.FAVICON_SIZE,
                        "crop": "fill",
                    },
                    {"quality": "auto:best"},
                    {"fetch_format": "auto"},
            ...
            url = result.get("secure_url")
            logger.info(
                f"Favicon uploaded successfully for tenant {tenant_subdomain}: {url}"
            )
