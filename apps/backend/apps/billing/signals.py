...
def auto_start_trial_for_new_tenant(sender, instance, created, **kwargs):
    ...
    existing = Subscription.objects.all_tenants().filter(client=instance).exists()
    ...
    trial_plan = Plan.objects.filter(
        is_active=True, trial_days__gt=0, slug="professional"  # Prefere o Professional
    ).first()

    if not trial_plan:
        # Fallback para qualquer plano com trial
        trial_plan = (
            Plan.objects.filter(is_active=True, trial_days__gt=0)
