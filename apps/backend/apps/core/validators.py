...
def validate_subdomain(value: str) -> None:
    ...
    if value.lower() in reserved:
        raise ValidationError(
            f'O subdomínio "{value}" está reservado e não pode ser usado'
        )
...
def validate_hex_color(value: str) -> None:
    ...
    if not re.match(pattern, value):
        raise ValidationError("Cor deve estar no formato hexadecimal (ex: #FF5733)")
...
def validate_protocol_code(value: str) -> None:
    ...
    if not re.match(pattern, value.upper()):
        raise ValidationError(
            "Código de protocolo inválido. " "Formato esperado: OUVY-XXXX-YYYY"
        )
