...
class EmailService:
    ...
    def send_feedback_notification(feedback: Feedback):
        ...
        try:
            ...
            if not feature_flags.is_enabled("EMAIL_NOTIFICATIONS"):
                logger.info(
                    f"📧 Notificações por email desabilitadas | Tenant: {tenant.nome} | Protocolo: {feedback.protocolo}"
                )
                return
            ...
            if not hasattr(tenant, "email_contato") or not tenant.email_contato:
                logger.warning(
                    f"Tenant {tenant.nome} não tem email configurado para notificações"
                )
                return
            ...
            context = {
                ...
                "data_criacao": timezone.localtime(feedback.data_criacao).strftime(
                    "%d/%m/%Y %H:%M"
                ),
            }
            ...
            logger.info(
                f"✅ Notificação de feedback enviada para {tenant.email_contato} | Protocolo: {feedback.protocolo}"
            )
    ...
    def send_feedback_status_update(feedback: Feedback, old_status: str):
        ...
        try:
            ...
            logger.info(
                f"✅ Notificação de status enviada para {tenant.email_contato} | Protocolo: {feedback.protocolo}"
            )
    ...
    def send_weekly_report(tenant: TenantClient):
        ...
        try:
            ...
            logger.info(
                f"✅ Relatório semanal enviado para {tenant.email_contato} | Tenant: {tenant.nome}"
            )
    ...
    def send_feedback_webhook(feedback: Feedback, event_type: str = "feedback.created"):
        ...
        try:
            ...
            if not feature_flags.is_enabled("WEBHOOKS"):
                logger.info(
                    f"🔗 Webhooks desabilitados | Tenant: {tenant.nome} | Evento: {event_type} | Protocolo: {feedback.protocolo}"
                )
                return
            ...
            headers = {
                ...
                "X-Ouvify-Signature": get_random_string(
                    32
                ),  # Em produção, implementar HMAC
            }

            response = requests.post(
                tenant.webhook_url, json=payload, headers=headers, timeout=10
            )

            if response.status_code == 200:
                logger.info(
                    f"✅ Webhook enviado com sucesso | URL: {tenant.webhook_url} | Evento: {event_type}"
                )
            else:
                logger.warning(
                    f"⚠️ Webhook falhou | Status: {response.status_code} | URL: {tenant.webhook_url}"
                )
