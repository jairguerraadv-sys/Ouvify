...
class TwoFactorAuthService:
    ...
    def setup_2fa(self) -> TwoFactorSetupResult:
        ...
        provisioning_uri = totp.provisioning_uri(
            name=self.user.email, issuer_name=self.ISSUER_NAME
        )
    ...
    def verify_totp(self, code: str, secret: str) -> TwoFactorVerifyResult:
        ...
        if is_valid:
            ...
            return TwoFactorVerifyResult(
                success=True, message="Código verificado com sucesso."
            )
    ...
    def verify_backup_code(
        self, code: str, stored_codes_hash: str
...
class TwoFactorUserMixin:
    ...
    def verify_2fa_code(self, code: str) -> TwoFactorVerifyResult:
        ...
        if not self.two_factor_enabled:
            return TwoFactorVerifyResult(
                success=True, message="2FA não está habilitado"
            )
        ...
        if len(code) == 6 and code.isdigit():
            if not self.two_factor_secret:
                return TwoFactorVerifyResult(
                    success=False, message="Configuração 2FA incompleta"
                )
        ...
        if self.two_factor_backup_codes:
            success, new_hash = service.verify_backup_code(
                code, self.two_factor_backup_codes
            )
        ...
        return TwoFactorVerifyResult(success=False, message="Código inválido.")
