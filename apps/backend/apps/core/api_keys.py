...
class APIKey(TenantAwareModel):
    ...
    requests_count = models.PositiveIntegerField(
        default=0,
        verbose_name='Total de Requisições'
    )
    ...
    @classmethod
    def generate(cls, client, name: str, permissions: str = 'read', 
                 expires_days: int = None, rate_limit: int = 1000):
        """
    ...
    def is_rate_limited(self) -> bool:
        ...
        if current >= self.rate_limit:
            logger.warning(f"⚠️ Rate limit excedido: {self.name} ({current}/{self.rate_limit})")
            return True
...
class APIKeyAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if key_instance.is_rate_limited():
            raise AuthenticationFailed('Rate limit excedido. Tente novamente mais tarde.')
