...
class CSPViolation(models.Model):
    ...
    document_uri = models.URLField(
        max_length=500,
        help_text="URI do documento que violou CSP (normalizada)"
    )
    violated_directive = models.CharField(
        max_length=100,
        help_text="Diretiva CSP que foi violada"
    )
    effective_directive = models.CharField(
        max_length=100,
        help_text="Diretiva efetiva que foi violada"
    )
    original_policy = models.TextField(
        help_text="Política CSP original"
    )
    ...
    source_file = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="Arquivo fonte (sanitizado)"
    )
    line_number = models.PositiveIntegerField(
        blank=True,
        null=True,
        help_text="Número da linha onde ocorreu a violação"
    )
