...
class AnalyticsDashboardView(APIView):
    ...
    def get(self, request):
        if not feature_flags.is_enabled('ANALYTICS'):
            return Response({'error': 'Analytics desabilitado para este ambiente'}, status=403)
        ...
        base_qs = Feedback.objects.filter(data_criacao__gte=start_date, data_criacao__lte=end_date)

        # Summary
        total_feedbacks = base_qs.count()

        feedbacks_with_first_response = base_qs.exclude(tempo_primeira_resposta__isnull=True)
        ...
        if feedbacks_with_first_response.exists():
            ...
            avg_response_time_hours = total_hours / max(feedbacks_with_first_response.count(), 1)
        ...
        rt_qs = Feedback.objects.filter(data_criacao__gte=rt_start, data_criacao__lte=rt_end).exclude(
            tempo_primeira_resposta__isnull=True
        )

        daily = (
            rt_qs.annotate(day=TruncDate('data_criacao'))
            .values('day')
            .annotate(
                total=Count('id'),
            )
        ...
        for delta in range(response_days):
            ...
            if day_feedbacks.exists():
                ...
                if count:
                    tempo = total / count
            response_time.append({'day': label, 'tempo': round(tempo, 2), 'meta': meta_hours})
        ...
        if period == 'week':
            ...
            for i in range(5, -1, -1):
                ...
                label = self.MONTH_LABELS_PT.get(month_start.month, str(month_start.month))
                trend.append(
                    {
                        'month': label,
                        'denuncias': counts['denuncias'],
