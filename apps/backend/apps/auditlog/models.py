...
class AuditLog(models.Model):
    ...
    action = models.CharField(
        "Ação", max_length=50, choices=ACTION_CHOICES, db_index=True
    )
    ...
    object_repr = models.CharField(
        "Representação do Objeto", max_length=200, blank=True
    )

    # Informações de contexto
    ip_address = models.GenericIPAddressField(
        "Endereço IP", null=True, blank=True, db_index=True
    )
    ...
    def __str__(self):
        ...
        return (
            f"[{self.timestamp:%Y-%m-%d %H:%M}] {user_str}: {self.get_action_display()}"
        )
...
class AuditLogSummary(models.Model):
    ...
    unique_users = models.PositiveIntegerField("Usuários Únicos", default=0)
...
class UserSession(models.Model):
    ...
    last_activity = models.DateTimeField("Última Atividade", default=timezone.now)
    ...
    device_type = models.CharField("Tipo de Dispositivo", max_length=20, blank=True)
    browser = models.CharField("Navegador", max_length=50, blank=True)
    os = models.CharField("Sistema Operacional", max_length=50, blank=True)
