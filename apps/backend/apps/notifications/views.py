...
class PushSubscriptionViewSet(viewsets.ModelViewSet):
    ...
    def unsubscribe(self, request):
        ...
        if not endpoint:
            return Response(
                {"error": "Endpoint é obrigatório"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response({"message": f"{count} subscription(s) desativada(s)"})
...
class NotificationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def send(self, request):
        ...
        logger.info(
            f"Admin {request.user.email} enviou {len(notifications)} notificações"
        )
