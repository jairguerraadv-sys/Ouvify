...
class PushSubscription(models.Model):
    ...
    endpoint = models.TextField(
        ...
        help_text='URL do Push Service'
    )
    p256dh = models.CharField(
        max_length=255, 
        verbose_name='P256DH Key',
        help_text='Chave pública ECDH P-256'
    )
    auth = models.CharField(
        max_length=255, 
        verbose_name='Auth Secret',
        help_text='Secret de autenticação'
    )
    ...
    active = models.BooleanField(
        default=True,
        verbose_name='Ativo',
        help_text='Se a subscription ainda é válida'
    )
...
class Notification(models.Model):
    ...
    body = models.TextField(
        verbose_name='Corpo',
        help_text='Texto principal da notificação'
    )
    icon = models.URLField(
        blank=True,
        default='',
        verbose_name='Ícone',
        help_text='URL do ícone a exibir'
    )
    ...
    clicked_at = models.DateTimeField(
        null=True, 
        blank=True,
        verbose_name='Clicada em'
    )
    
    # Estatísticas de entrega
    delivery_success = models.BooleanField(
        default=False,
        verbose_name='Entrega bem-sucedida'
    )
    delivery_error = models.TextField(
        blank=True,
        default='',
        verbose_name='Erro de Entrega'
    )
    ...
    def record_delivery(self, success: bool, error: str = '', push_count: int = 0) -> None:
        """Registra resultado da entrega"""
        self.delivery_success = success
        self.delivery_error = error
        self.push_sent_count = push_count
        self.save(update_fields=['delivery_success', 'delivery_error', 'push_sent_count'])
...
class NotificationPreference(models.Model):
    ...
    notify_feedback_novo = models.BooleanField(
        default=True,
        verbose_name='Novos Feedbacks'
    )
    notify_feedback_atualizado = models.BooleanField(
        default=True,
        verbose_name='Feedbacks Atualizados'
    )
    notify_feedback_comentario = models.BooleanField(
        default=True,
        verbose_name='Novos Comentários'
    )
