...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            self.stdout.write(
                self.style.ERROR(
                    'py_vapid não está instalado. Execute: pip install py_vapid'
                )
            )
            return
        ...
        # Versão simplificada sem py_vapid
    ...
    def _generate_simple_keys(self, output_format):
        ...
        public_base64 = base64.urlsafe_b64encode(public_bytes).rstrip(b'=').decode('utf-8')
