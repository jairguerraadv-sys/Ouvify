...
def send_push_notification(self, notification_id: int) -> Dict[str, Any]:
    ...
    try:
        notification = Notification.objects.select_related("user", "tenant").get(
            id=notification_id
        )
    ...
    if preferences:
        if not preferences.should_notify(notification.tipo):
            logger.info(
                f"Usuário {user.id} desabilitou notificações do tipo {notification.tipo}"
            )
    ...
    subscriptions = PushSubscription.objects.filter(
        user=user, tenant=notification.tenant, active=True
    )
    ...
    for subscription in subscriptions:
        try:
            ...
            if e.response and e.response.status_code in (404, 410):
                ...
                logger.info(
                    f"Subscription {subscription.id} desativada (status {e.response.status_code})"
                )
    ...
    logger.info(
        f"Push notification {notification_id}: {success_count} sucesso, {error_count} erros"
    )
...
def send_feedback_created_push(feedback_id: int) -> Dict[str, Any]:
    ...
    admins = User.objects.filter(tenant=tenant, is_staff=True, is_active=True)
    ...
    logger.info(
        f"Notificações de novo feedback enviadas para {notifications_sent} admins"
    )
    return {"notifications_sent": notifications_sent}


@shared_task
def send_status_update_push(
    feedback_id: int, old_status: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    status_display = (
        ...
        if hasattr(feedback, "get_status_display")
        else new_status
    )
    ...
    logger.info(
        f"Notificação de atualização enviada para autor do feedback {feedback_id}"
    )
...
def send_broadcast_notification(
    tenant_id: int, title: str, body: str, url: str = "", data: Optional[Dict] = None
) -> Dict[str, Any]:
    """
    ...
    logger.info(
        f"Broadcast enviado para {notifications_created} usuários do tenant {tenant_id}"
    )
...
def cleanup_old_notifications(days: int = 90) -> Dict[str, int]:
    ...
    deleted_count, _ = Notification.objects.filter(
        sent_at__lt=cutoff, read_at__isnull=False
    ).delete()
...
def cleanup_inactive_subscriptions(days: int = 30) -> Dict[str, int]:
    ...
    deleted_count, _ = PushSubscription.objects.filter(active=False).delete()

    # Também deletar subscriptions nunca usadas e muito antigas
    old_unused, _ = PushSubscription.objects.filter(
        last_used__isnull=True, created_at__lt=cutoff
    ).delete()
