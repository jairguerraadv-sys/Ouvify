...
class FeedbackFilter(filters.FilterSet):
    ...
    assigned_to = filters.NumberFilter(
        field_name="assigned_to", help_text="Filtrar por ID do TeamMember atribuído"
    )
    ...
    unassigned = filters.BooleanFilter(
        method="filter_unassigned", help_text="true = apenas feedbacks não atribuídos"
    )
    ...
    def filter_assigned_to_me(self, queryset, name, value):
        ...
        try:
            ...
            team_member = TeamMember.objects.get(
                user=user, client=tenant, status=TeamMember.ACTIVE
            )
