...
def _round_robin_assignment(feedback):
    ...
    members = list(TeamMember.objects.filter(
        ...
        role=TeamMember.VIEWER  # Viewers não recebem atribuições
    ).order_by('id'))
    
    if not members:
        logger.warning(f"⚠️ Nenhum membro disponível para atribuição no tenant {feedback.client.nome}")
        return
...
def _generate_tenant_digest(tenant):
    ...
    stats = Feedback.objects.filter(client=tenant).aggregate(
        ...
        resolvidos_ontem=Count('id', filter=Q(
            data_resolucao__gte=ontem,
            status='resolvido'
        )),
        sla_vencidos=Count('id', filter=Q(
            sla_primeira_resposta=False
        ) | Q(
            sla_resolucao=False
        ))
    )
