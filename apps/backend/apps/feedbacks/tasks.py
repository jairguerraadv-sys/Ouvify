...
def send_assignment_email(self, feedback_id: int, team_member_id: int):
    ...
    try:
        ...
        if hasattr(team_member, 'email_notifications') and not team_member.email_notifications:
        ...
        html_message = render_to_string('emails/feedback_assigned.html', {
            ...
            'assigned_by': feedback.assigned_by.get_full_name() if feedback.assigned_by else 'Sistema',
        })
...
def send_new_feedback_email(self, feedback_id: int):
    ...
    try:
        ...
        emails_sent = 0
        for admin in admins:
            html_message = render_to_string('emails/new_feedback.html', {
                'admin': admin,
                'feedback': feedback,
                'feedback_url': feedback_url,
            })
