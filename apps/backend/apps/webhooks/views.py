...
class WebhookEndpointViewSet(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        ...
        stats = {
            ...
            "total_deliveries_today": WebhookDelivery.objects.filter(
                endpoint__in=endpoints, created_at__gte=start_of_day
            ).count(),
    ...
    def available_events(self, request):
        ...
        return Response(
            [{"value": e[0], "label": e[1]} for e in WebhookEndpoint.EVENTS]
        )
...
class WebhookEventViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return WebhookEvent.objects.filter(payload__tenant_id=tenant.id).order_by(
            "-created_at"
        )[:100]
...
class WebhookDeliveryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return WebhookDelivery.objects.filter(endpoint__client=tenant).order_by(
            "-created_at"
        )[:100]
    ...
    def retry(self, request, pk=None):
        ...
        success = deliver_webhook(
            delivery.endpoint, delivery.event, delivery.attempt + 1
        )
