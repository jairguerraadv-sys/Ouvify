...
def create_webhook_event(
    event_type: str, payload: dict, source_model: str = "", source_id: str = ""
) -> WebhookEvent:
    """
...
def deliver_webhook(
    endpoint: WebhookEndpoint, event: WebhookEvent, attempt: int = 1
) -> bool:
    """
...
def schedule_retry(
    ...
    delay_seconds = endpoint.retry_delay * (
        2 ** (current_attempt - 1)
    )  # Exponential backoff
    ...
    retry_webhook_delivery.apply_async(
        args=[str(endpoint.id), str(event.id), current_attempt + 1], eta=next_retry
    )
...
def trigger_feedback_created(feedback):
    ...
    payload = {
        ...
        "feedback": {
            ...
            "data_criacao": (
                feedback.data_criacao.isoformat() if feedback.data_criacao else None
            ),
        },
    }
    return create_webhook_event(
        "feedback.created", payload, "Feedback", str(feedback.id)
    )
...
def trigger_feedback_status_changed(feedback, old_status, new_status):
    ...
    return create_webhook_event(
        "feedback.status_changed", payload, "Feedback", str(feedback.id)
    )
...
def trigger_feedback_resolved(feedback):
    ...
    payload = {
        ...
        "feedback": {
            ...
            "data_resolucao": (
                feedback.data_resolucao.isoformat() if feedback.data_resolucao else None
            ),
        },
    }
    return create_webhook_event(
        "feedback.resolved", payload, "Feedback", str(feedback.id)
    )
