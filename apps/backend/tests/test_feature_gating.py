...
from apps.feedbacks.constants import FeedbackStatus, InteracaoTipo
...
class ClientPlanMethodsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.client_pro = Client.objects.create(
            owner=self.user, nome="Pro Company", subdominio="pro-company", plano="pro"
        )
...
class FeedbackViewSetFeatureGatingTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user_empresa = User.objects.create_user(
            username="empresa_teste", password="pass123"
        )
    ...
    def test_free_plan_cannot_create_internal_notes(self):
        ...
        response = self.api_client.post(
            ...
            {"mensagem": "Esta é uma nota interna", "tipo": "NOTA_INTERNA"},
            format="json",
        )
    ...
    def test_starter_plan_can_create_internal_notes(self):
        ...
        interacao = FeedbackInteracao.objects.all_tenants().get(
            feedback=self.feedback_starter, tipo=InteracaoTipo.NOTA_INTERNA
        )
...
class FeatureGatingExceptionHandlerTestCase(APITestCase):
    ...
    def test_feature_not_available_returns_403(self):
        ...
        response = self.api_client.post(
            ...
            {"mensagem": "Nota interna bloqueada", "tipo": "NOTA_INTERNA"},
            format="json",
        )
