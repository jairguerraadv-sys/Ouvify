...
class TestFeedbackAssignment:
    ...
    def test_assign_feedback_to_team_member(self):
        ...
        client = Client.objects.create(
            nome="Test Company", subdominio=get_unique_subdominio()
        )
        admin_user = User.objects.create_user(
            f"admin-{uuid.uuid4().hex[:8]}@test.com", password="Test@1234"
        )
        moderator_user = User.objects.create_user(
            f"mod-{uuid.uuid4().hex[:8]}@test.com", password="Test@1234"
        )
    ...
    def test_filter_assigned_to_me(self):
        ...
        user1 = User.objects.create_user(
            f"user1-{uuid.uuid4().hex[:8]}@test.com", password="Test@1234"
        )
        user2 = User.objects.create_user(
            f"user2-{uuid.uuid4().hex[:8]}@test.com", password="Test@1234"
        )

        member1 = TeamMember.objects.create(
            user=user1, client=client, role=TeamMember.MODERATOR
        )
        member2 = TeamMember.objects.create(
            user=user2, client=client, role=TeamMember.MODERATOR
        )
        ...
        my_feedbacks = Feedback.objects.all_tenants().filter(
            client=client, assigned_to=member1
        )
    ...
    def test_unassign_feedback(self):
        ...
        user = User.objects.create_user(
            f"user-{uuid.uuid4().hex[:8]}@test.com", password="Test@1234"
        )
        member = TeamMember.objects.create(
            user=user, client=client, role=TeamMember.MODERATOR
        )
    ...
    def test_filter_unassigned_feedbacks(self):
        ...
        unassigned = Feedback.objects.all_tenants().filter(
            client=client, assigned_to__isnull=True
        )
    ...
    def test_multiple_assignments_to_same_member(self):
        ...
        assigned = Feedback.objects.all_tenants().filter(
            client=client, assigned_to=member
        )
