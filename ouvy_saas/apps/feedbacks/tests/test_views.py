...
class TestFeedbackViewSet:
    ...
    def test_list_feedbacks_isolation(self, authenticated_client, tenant, tenant2, user, user2):
        ...
        if 'results' in response.data:
            for fb in response.data['results']:
                assert fb.get('client') == tenant.id or 'Tenant 2' not in fb.get('titulo', '')
    ...
    def test_retrieve_feedback_from_other_tenant_fails(self, api_client, tenant, tenant2, user2, feedback):
    ...
    def test_add_interacao_to_feedback(self, authenticated_client, tenant, feedback, user):
        """Teste adicionar interação ao feedback"""
        url = reverse('feedback-adicionar-interacao', kwargs={'pk': feedback.pk})
        data = {
            'mensagem': 'Resposta ao feedback',
            'tipo': 'resposta'
        }
